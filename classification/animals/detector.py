...
def initialize_detector(model_path="yolov8n.pt"):
    ...
    try:
        print("Initializing YOLOv8 Nano detector for animals...")
...
def detect_animals(detector, image_path, confidence_threshold=0.5):
...
def process_images(input_folder, output_folder=None, confidence_threshold=0.5):
    """Process images in a folder for animal detection."""
    ...
    for image_path in image_files:
        try:
            detections = detect_animals(detector, image_path, confidence_threshold)
            stats['processed'] += 1
            if detections:
...
def print_summary(start_time, stats, images_with_objects):
    """Print summary statistics for animal detection."""
