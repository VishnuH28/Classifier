...
def initialize_detector(model_path="yolov8n.pt"):
    ...
    try:
        print("Initializing YOLOv8 Nano detector for plants...")
...
def detect_plants(detector, image_path, confidence_threshold=0.5):
    ...
    try:
        ...
        for result in results:
            for box in result.boxes:
                ...
                if class_id in PLANT_CLASSES and confidence > confidence_threshold:
                    label = PLANT_CLASSES[class_id]
...
def process_images(input_folder, output_folder=None, confidence_threshold=0.5):
    """Process images in a folder for plant detection."""
    ...
    for image_path in image_files:
        try:
            detections = detect_plants(detector, image_path, confidence_threshold)
            stats['processed'] += 1
            if detections:
...
def print_summary(start_time, stats, images_with_objects):
    """Print summary statistics for plant detection."""
